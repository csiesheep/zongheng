...
export default {
  ...
  sheet: {
    ...
    // #118 round 3(orchestrator,真的牌局:手上三張楚牌全部可搭配 + 軍師開著
    // 真的釘住 banner,把這句擠到 .sheet-mid 的折線之下,375/390 en 都一樣):
    // explain 整句搬出 mid,改由下面 hint.shuoke(釘住、永不捲動)一句話帶完整
    // 規則——牌怎麼玩不再需要 mid 有沒有位置。這個 key 保留(沒刪),不再呼叫。
    shuoke: {
    ...
    hint: {
      ...
      // 玩法是搭配一張對手的牌。#118 round 3:這一句釘住的提示現在是唯一
      // 講這條規則的地方(原本 mid 裡那句更長的 explain 不一定有位置放),
      // 一行,永遠在畫面上。
      shuoke: "說客:搭配一張{enemy}的牌,用它的行動點,它的事件不觸發。",
      enemy: "對手的牌:行動點照用,但事件也會發生;先後由你選。",
      score: "記分卡:本回合內一定要打出。",
      // #34:唯讀牌頁自己的一行說明(owner 授權文案);{side} 是打出/棄掉/
      // 蓋標題牌的那一方,不是目前看牌的玩家自己那一邊。
      played: "{side}打出的牌。",
      // #122:對手的牌選了先事件,而事件會降低這張牌自己的行動點(秦打荊軻刺秦王:
      // 2 − 1)時,取代上面 enemy 那一句;{ops} 是事件結算後真正能用的點數。
      enemyOps: {
    ...
    history: "史事",
    source: "出處:",
    // #122:牌文本身讀法有疑義時的裁定(owner 裁決 #119「其他照牌文字面改」),
    // 以卡牌 id 為 key。牌的詳細頁(card-view.js:手機遮罩、桌面側欄、對局中的
    // 看牌)與規則頁的卡牌清單(rules.js)都從這裡讀,只有一份。
    rulings: {
